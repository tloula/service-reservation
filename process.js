...
jQuery(".request-form").submit(function (event) {
  ...
  if (honeypot == "") {
    ...
    var posting = jQuery.post(
      ...
      function (data) {
        ...
        if (data.status == 0) {
          jQuery(id).attr(
            "class",
            "fusion-alert alert error alert-danger fusion-alert-center fusion-default-alert-style error fusion-alert-capitalize alert-dismissable alert-shadow"
          );
        } else {
          jQuery(id).attr(
            "class",
            "fusion-alert alert success alert-success fusion-alert-center fusion-default-alert-style success fusion-alert-capitalize alert-dismissable alert-shadow"
          );
        }
      },
      "json"
    ...
    jQuery(id)
      .empty()
      .append(
        "Suspicious activity detected. Please try without an auto form filler."
      );
    jQuery(id).attr(
      "class",
      "fusion-alert alert error alert-danger fusion-alert-center fusion-default-alert-style error fusion-alert-capitalize alert-dismissable alert-shadow"
